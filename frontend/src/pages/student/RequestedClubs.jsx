            ...
            {!isLoading && (
                <Grid container spacing={3} mb={3}>
                    {requestedClubs?.map(club => {
                        const { _id, name, description, purpose, member_fees, founded_date, email, phone_number } = club;
                                    ...
                                    <CardContent>
                                        <Box mt={.5} display="flex" flexDirection="column" rowGap={1}>
                                            ...
                                            <Box display="flex" alignItems="center" columnGap={1}>
                                                ...
                                                <Typography variant="body2" color="site.text">
                                                    {clubs?.find(club => club._id === _id)?.members.filter(member => member.request && member.approve).length} members
                                                </Typography>
                                            </Box>
                                            <Button variant="outlined" sx={{ mt: 1 }} onClick={() => {
                                                dispatch(cancel({ _id, student: authUser._id }));
